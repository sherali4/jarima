...
def tekshirish(soato4, inn, xat_turi, hisobot_turi, yil, aniqlangan_sana, xat_sanasi, fayl_nomi):
    ...
    ########################QRCODE##############################################
    # Sana topish uchun regex (1-2 raqam + o‘zbekcha oy + 4 raqamli yil)
    
    text_clean = text.replace('“', '').replace('”', '').replace('–', '-')
    match = re.search(r'\b(\d{1,2})\s+(yanvar|fevral|mart|aprel|may|iyun|iyul|avgust|sentabr|oktabr|noyabr|dekabr)\s+(\d{4})', text_clean.lower())
    if match:
        kun, oy_uz, yil = match.groups()

        # O‘zbekcha oylarni inglizchaga o‘girish
        oylar = {
            'yanvar': 'January', 'fevral': 'February', 'mart': 'March', 'aprel': 'April',
            'may': 'May', 'iyun': 'June', 'iyul': 'July', 'avgust': 'August',
            'sentabr': 'September', 'oktabr': 'October', 'noyabr': 'November', 'dekabr': 'December'
        }

        sana_str = f"{kun} {oylar[oy_uz]} {yil}"
        sana_obj = datetime.strptime(sana_str, "%d %B %Y")
        sana_formatted = sana_obj.strftime("%d.%m.%Y")

    if mavjud['xat_sanasi'] == sana_formatted:
        baza['xat_sanasi'] =True
