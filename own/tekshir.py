...
def tekshirish(soato4, inn, xat_turi, hisobot_turi, yil, aniqlangan_sana, xat_sanasi, fayl_nomi):
    ...
    xat_kun = str(xat_sanasi[:2])
    xat_oy = str(xat_sanasi[3:5])
    xat_yil = str(xat_sanasi[6:10])
    ...
    regex = re.compile(fr"{xat_yil}.*?{xat_oy}.*?{xat_kun}")  # f-string + raw-string
    regex1 = re.compile(fr"{xat_kun}.*?{xat_oy}.*?{xat_yil}")  # f-string + raw-string
    match = regex.search(text_clean)
    
    if regex.search(text_clean):
        baza['xat_sanasi'] = True
    
    if regex1.search(text_clean):
        baza['xat_sanasi'] = True
